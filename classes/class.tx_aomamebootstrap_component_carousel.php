 ...
 */
class tx_aomamebootstrap_component_carousel extends tslib_pibase {
	public $prefixId      = 'tx_aomamebootstrap_component_carousel';		// Same as class name
	public $scriptRelPath = 'classes/class.tx_aomamebootstrap_component_carousel.php';	// Path to this script relative to the extension dir.
	...
	private function configuration(){
		...
		$getContentUidList = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'carousel_contentuidlist', 'sDEF');
		$this->contentUidList = $getContentUidList ? $getContentUidList:$this->conf['bootstrap.']['carousel.']['contentUidList'];
		
		$getCarouselHtmlId = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'carousel_carouselhtmlid', 'sDEF');
		$this->carouselHtmlId = $getCarouselHtmlId ? $getCarouselHtmlId:$this->conf['bootstrap.']['carousel.']['carouselHtmlId'];
		
		$getBootstrapSpan = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'carousel_bootstrapspan', 'sDEF');
		$this->bootstrapSpan = $getBootstrapSpan ? $getBootstrapSpan:$this->conf['bootstrap.']['carousel.']['bootstrapSpan'];
		
		$getBootstrapRow = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'carousel_bootstraprow', 'sDEF');
		$this->bootstrapRow = $getBootstrapRow ? $getBootstrapRow:$this->conf['bootstrap.']['carousel.']['bootstrapRow'];
		
		$getShowText = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'carousel_showtext', 'sDEF');
		$this->showText = $getShowText ? $getShowText:$this->conf['bootstrap.']['carousel.']['showText'];
		
		$getInterval = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'carousel_interval', 'sDEF');
		$this->interval = $getInterval ? $getInterval:$this->conf['bootstrap.']['carousel.']['interval'];
		
		$getStopOnHover = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'carousel_stoponhover', 'sDEF');
