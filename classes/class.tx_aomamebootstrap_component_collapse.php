 ...
 */
class tx_aomamebootstrap_component_collapse extends tslib_pibase {
	public $prefixId      = 'tx_aomamebootstrap_component_collapse';		// Same as class name
	public $scriptRelPath = 'classes/class.tx_aomamebootstrap_component_collapse.php';	// Path to this script relative to the extension dir.
	...
	private function configuration(){
		...
		$getContentUidList = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'collapse_contentuidlist', 'sDEF');
		$this->contentUidList = $getContentUidList ? $getContentUidList:$this->conf['bootstrap.']['collapse.']['contentUidList'];
		
		$getAccordingHtmlId = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'collapse_accordinghtmlid', 'sDEF');
		$this->accordingHtmlId = $getAccordingHtmlId ? $getAccordingHtmlId:$this->conf['bootstrap.']['collapse.']['accordingHtmlId'];
		
		$getBootstrapSpan = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'collapse_bootstrapspan', 'sDEF');
		$this->bootstrapSpan = $getBootstrapSpan ? $getBootstrapSpan:$this->conf['bootstrap.']['collapse.']['bootstrapSpan'];
		
		$getBootstrapRow = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'collapse_bootstraprow', 'sDEF');
		$this->bootstrapRow = $getBootstrapRow ? $getBootstrapRow:$this->conf['bootstrap.']['collapse.']['bootstrapRow'];
		
		$getCollapseStyle = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'collapse_collapsestyle', 'sDEF');
...
if (defined('TYPO3_MODE') && isset($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/aomame_bootstrap/classes/class.tx_aomamebootstrap_component_collapse.php'])) {
	include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/aomame_bootstrap/classes/class.tx_aomamebootstrap_component_collapse.php']);
}
