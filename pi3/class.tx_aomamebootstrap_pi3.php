 ...
 */
class tx_aomamebootstrap_pi3 extends tslib_pibase {
	public $prefixId      = 'tx_aomamebootstrap_pi3';		// Same as class name
	public $scriptRelPath = 'pi3/class.tx_aomamebootstrap_pi3.php';	// Path to this script relative to the extension dir.
	...
	private function configuration(){
		...
		$getContentUid = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'contentuid', 'sDEF');
		$this->contentUid = $getContentUid?$getContentUid:$this->conf['bootstrap.']['popover.']['contentUid'];
		
		$getDirection = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'direction', 'sDEF');
		$this->direction = $getDirection?$getDirection:$this->conf['bootstrap.']['popover.']['direction'];
		
		$getSelector = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'selector', 'sDEF');
		$this->selector = $getSelector?$getSelector:$this->conf['bootstrap.']['popover.']['selector'];
		
		$getTrigger = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'trigger', 'sDEF');
		$this->trigger = $getTrigger?$getTrigger:$this->conf['bootstrap.']['popover.']['trigger'];
		
		$getDelayShow = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'delayshow', 'sDEF');
		$this->delayShow = $getDelayShow?$getDelayShow:$this->conf['bootstrap.']['popover.']['delayShow'];
		
		$getDelayHide = $this->pi_getFFvalue($this->cObj->data['pi_flexform'], 'delayhide', 'sDEF');
